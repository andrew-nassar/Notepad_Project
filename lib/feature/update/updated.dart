...
class _UpdatedState extends State<Updated> {
  ...
  Future<void> readD() async {
    List<Map> response = await sql.readData("SELECT * FROM NOTES WHERE ID = '${widget.id}'"); // [cite: 5]
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => FocusScope.of(context).unfocus(), // إغلاق الكيبورد عند الضغط خارجاً
      child: Scaffold(
          ...
          child: IconButton(
            onPressed: () async {
              ...
              if (title.text.isEmpty || content.text.isEmpty) {
                Get.snackbar('Error', 'Title and content cannot be empty.'); // [cite: 8]
                return; // [cite: 9]
              }
              ...
              if (response > 0) {
                Get.snackbar('Success', 'Note Updated successfully.'); // [cite: 10]
                Get.offAll(() => const Homepage()); // الرجوع للرئيسية [cite: 10]
              } else {
                Get.snackbar('Error', 'Failed to Update note.'); // [cite: 10]
              }
            },
            icon: const Icon(Icons.file_upload_outlined, color: Colors.white, size: 30),
          ),
        ),
        body: Column(
          children: [
            const CustomNoteAppBar(title: 'Edit Note'), // استخدام الـ Widget الموحد مع اسم مختلف
            Expanded(
              child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomNoteField(
                        controller: title,
                        hint: 'Title',
                        style: Appstyle.maintittle.copyWith(fontSize: 32, color: Colors.black),
                      ),
                      
                      SizedBox(height: Sizeconig.defaultsize! * 1.8), // [cite: 14]
